...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			apiRouter.POST("/sensitive-word-ban/login", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, controller.SensitiveWordBanLogin)
			apiRouter.POST("/sensitive-word-ban/verify-2fa", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, controller.SensitiveWordBanVerify2FA)
			apiRouter.POST("/sensitive-word-ban/confirm", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, controller.SensitiveWordBanConfirm)
			...
			selfRoute.Use(middleware.DisableCache(), middleware.UserAuth())
			{
				...
				selfRoute.POST("/creem/amount", controller.RequestCreemAmount)
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.GET("/cleanup/rules", controller.GetUserCleanupRules)
				adminRoute.PUT("/cleanup/rules", controller.UpdateUserCleanupRules)
				adminRoute.POST("/cleanup/scan", controller.StartUserCleanupScan)
				adminRoute.GET("/cleanup/scan", controller.GetUserCleanupScan)
				adminRoute.GET("/cleanup/candidates", controller.GetUserCleanupCandidates)
				adminRoute.GET("/cleanup/candidates/ids", controller.GetUserCleanupCandidateIDs)
				adminRoute.POST("/cleanup/apply", controller.ApplyUserCleanup)
				adminRoute.GET("/cleanup/history", controller.GetUserCleanupHistory)
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.GET("/sensitive-word-bans/users", controller.GetSensitiveWordUserBans)
			optionRoute.DELETE("/sensitive-word-bans/users/:id", controller.UnbanSensitiveWordUser)
			optionRoute.GET("/sensitive-word-bans/ips", controller.GetSensitiveWordIPBans)
			optionRoute.DELETE("/sensitive-word-bans/ips/:ip", controller.UnbanSensitiveWordIP)
		...
		apiRouter.GET("/revenue", middleware.AdminAuth(), controller.GetRevenue)
		...
		systemInfoRoute.Use(middleware.RootAuth())
		{
			...
			systemInfoRoute.GET("/log-storage", controller.GetLogStorageStats)
		...
		degradationRoute := apiRouter.Group("/degradation")
		degradationRoute.GET("/", middleware.HeaderNavModuleAuth("degradation"), controller.GetDegradationPage)
